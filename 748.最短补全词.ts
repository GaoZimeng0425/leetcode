/*
 * @Author: gaozimeng
 * @Date: 2021-05-03 11:05:36
 * @LastEditors: gaozimeng
 * @LastEditTime: 2021-05-03 11:27:08
 * @Description: Do not edit
 * @FilePath: /.leetcode/748.最短补全词.ts
 */
/*
...
const calculate = (target, source) => {
  for (const key in target) {
    if (!source[key] || target[key] > source[key]) return false
  }
  return true
}
function shortestCompletingWord(licensePlate: string, words: string[]): string {
  let r = count(licensePlate)
  let min: string = ''
  for (const word of words) {
    console.log(word)
    if (!calculate(r, count(word))) continue
    console.log(word)
    if (min === ''|| min.length > word.length) {
      min = word
    }
  }
  return min
