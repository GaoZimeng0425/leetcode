/*
 * @Author: gaozimeng
 * @Date: 2021-05-22 10:53:47
 * @LastEditors: gaozimeng
 * @LastEditTime: 2021-05-22 11:28:35
 * @Description: Do not edit
 * @FilePath: /undefined/Users/gaozimeng/.leetcode/937.重新排列日志文件.ts
 */
/*
...
let compare = (a: string, b: string): number => {
  ...
  return index === b.length ? 0 : -1
}
function reorderLogFiles(logs: string[]): string[] {
  logs.sort((a, b) => {
    const [prefix1, ...av] = a.split(' ')
    const [prefix2, ...bv] = b.split(' ')
    const log1String = av.join(' ')
    const log2String = bv.join(' ')
    const isDigit1 = !Number.isNaN(parseInt(av[0]))
    const isDigit2 = !Number.isNaN(parseInt(bv[0]))
    if (!isDigit1 && !isDigit2) {
      return compare(log1String, log2String) || compare(prefix1, prefix2)
    }
