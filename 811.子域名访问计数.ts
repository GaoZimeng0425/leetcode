/*
 * @Author: gaozimeng
 * @Date: 2021-05-09 12:55:31
 * @LastEditors: gaozimeng
 * @LastEditTime: 2021-05-09 13:30:17
 * @Description: Do not edit
 * @FilePath: /ssr/Users/gaozimeng/.leetcode/811.子域名访问计数.ts
 */
/*
...
function subdomainVisits(cpdomains: string[]): string[] {
  ...
  cpdomains.forEach(item => {
    const [count, fullURL] = item.split(' ')
    const domain = fullURL.split('.')
    const { length } = domain
    let href = ''
    for (let i = length - 1; i >= 0; i--) {
      const cur = domain[i]
      href = href ? `${cur}.${href}` : cur
      domainHash[href] = (domainHash[href] || 0) + parseInt(count)
    }
  })
  return Object.entries(domainHash).map(([key, value]) => `${value} ${key}`)
};
// @lc code=end
